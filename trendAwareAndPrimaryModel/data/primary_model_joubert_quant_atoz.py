...
def load_merged_data() -> pd.DataFrame:
    ...
    for key, file_path in RAW_FILES.items():
        safe_path = _ensure_within_project(file_path)
        if not safe_path.exists():
            raise FileNotFoundError(f"Missing required file: {safe_path}")
        alias = ASSET_ALIASES[key]
        parts.append(load_single_asset(safe_path, alias))
...
def save_outputs(
    ...
    safe_reports_dir = _ensure_within_project(REPORTS_DIR)
    safe_reports_dir.mkdir(parents=True, exist_ok=True)

    signal_path = _ensure_within_project(safe_reports_dir / "primary_jq_signal.csv")
    weights_path = _ensure_within_project(safe_reports_dir / "primary_jq_weights_history.csv")
    diag_path = _ensure_within_project(safe_reports_dir / "primary_jq_factor_diagnostics_latest.csv")
    summary_path = _ensure_within_project(safe_reports_dir / "primary_jq_summary.csv")
    usage_path = _ensure_within_project(safe_reports_dir / "primary_jq_factor_usage.csv")
    data_qc_path = _ensure_within_project(safe_reports_dir / "primary_jq_data_qc.csv")
    bias_qc_path = _ensure_within_project(safe_reports_dir / "primary_jq_bias_qc.csv")
    gate_path = _ensure_within_project(safe_reports_dir / "primary_jq_oos_gate.csv")
