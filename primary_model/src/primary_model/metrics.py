"""Performance metrics for monthly strategy return series."""
...
def annualized_return(r: pd.Series, periods_per_year: int = 12) -> float:
    """Compute geometric annualized return from periodic decimal returns."""
...
def annualized_vol(r: pd.Series, periods_per_year: int = 12) -> float:
    """Compute annualized volatility from periodic decimal returns."""
    clean = pd.to_numeric(r, errors="coerce").dropna()
...
def sharpe_ratio(
    ...
    """Compute annualized Sharpe ratio using excess returns over per-period rf."""
    clean = pd.to_numeric(r, errors="coerce").dropna()
...
def max_drawdown(equity: pd.Series) -> float:
    """Compute max drawdown as min(equity / equity.cummax() - 1)."""
...
def perf_table(
    ...
    """Build a strategy performance table from backtest outputs.

    Uses `net_return` and `equity_net` by default.
    """
    columns = [
        "ann_return",
        "ann_vol",
