...
def load_data(bcom_path, spx_path, treasury_path, ig_path):
    ...
    def load_single_file(filepath, index_name):
        ...
        data_df = data_df.loc[:, [c for c in data_df.columns if pd.notna(c)]]
        data_df.columns = [str(c).strip() for c in data_df.columns]

        date_col = next((c for c in data_df.columns if c.lower() == 'date'), None)
        price_col = next((c for c in data_df.columns if c.upper() == 'PX_LAST' or 'price' in c.lower() or 'close' in c.lower()), None)
        return_col = next((c for c in data_df.columns if c.upper() == 'CHG_PCT_1D' or 'return' in c.lower()), None)

        if date_col is None or price_col is None or return_col is None:
            raise ValueError(f"Missing required columns in {filepath}: Date/PX_LAST/CHG_PCT_1D")

        data_df = data_df[[date_col, price_col, return_col]].copy()
        ...
        data_df['Return'] = pd.to_numeric(data_df['Return'], errors='coerce') / 100.0
        data_df = data_df.dropna(subset=['Date']).drop_duplicates(subset=['Date'], keep='last')
...
if __name__ == "__main__":
    ...
    BCOM_PATH = DATA_DIR / 'bcom.xlsx'
    SPX_PATH = DATA_DIR / 'spx.xlsx'
    TREASURY_PATH = DATA_DIR / 'treasury_10y.xlsx'
    IG_PATH = DATA_DIR / 'corp_bonds.xlsx'
    ...
    output_path = ROOT_DIR / 'reports' / 'backtest_results.csv'
